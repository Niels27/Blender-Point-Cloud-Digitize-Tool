...
Optional is a web environment for the interface and implementing AI assistance (or at least making the toolset AI ready).
...
To develop faster, use in-blender script. To do so, open text-> full script.py. Then run with alt+p
...
Points percentage: changes the displayed point amount to help performance, does NOT modify the amount of points. It does affect points export to shapefile.

max z: cuts off height (meters), since we only need roads. Set to 0 to not cut. Improves performance a lot. (Recommended 0.5)

center point cloud: use to find the point cloud.

Create Point Cloud Object: creates a blender object of the cloud. if BlenderGIS add-on is installed, export to shapefile will work on this object.
...
- Blender can freeze when clicking with too low intensity_threshold.
...
- Import a point cloud manualy when enabling the add-on or else nothing works.

Extra notes:

- The included auto.laz has triangles, road lines, bicycle symbols, speed limit marking, speedbump markings, middle lines.
It is the fastest to test because of its small size. But it is missing: zebra crossing, arrow markings
- Webbased PoC (proof of concept) uses a JSON file to load a point cloud in a web enviroment, using Flask and Three.js. It does not work since JSON files of point clouds become way too long. (of course, this is why .las/.laz exists..)
