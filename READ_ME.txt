...
Made changes to add-on scripts? -> make new zip file of add-on folder, remove old add-on and install again. Often needs blender restart to prevent errors.

To develop faster, use in-blender script. To do so, open text-> full script.py. Then run with alt+p
To save time, it has Auto load auto.laz function, this will quickly imports a laz file called auto.laz on every script execution. Otherwise need to manually import again on every execution to get point cloud data back.
...
Use ''center point cloud'' to find the point cloud.
Use ''Create Point Cloud Object'' to create blender object of the cloud. if BlenderGIS add-on is installed, export to shapefile works on this object.
...
! important press ESC key  to stop drawing/marker functions. !
