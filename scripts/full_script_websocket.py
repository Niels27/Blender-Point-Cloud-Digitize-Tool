...
class FixedTriangleMarkOperator(bpy.types.Operator):
    ...
    #Add a property to store external coordinates
    external_location = bpy.props.FloatVectorProperty(name="External Location", size=3, default=(0.0, 0.0, 0.0))
    ...
    # Method to handle external coordinates
    def execute_with_location(self, context, location):
        self.external_location = location
        print("drawing fixed triangle")
        draw_fixed_triangle(context, self.external_location, size=0.5)
        print("finished drawing fixed triangle")
        return {'FINISHED'}
    ...
    while True:
        ...
        try:
            ...
            if "x" in coords and "y" in coords and "z" in coords:
                print("Received data containes x,y,z coordinates")
                # Create an operator instance and set the external location
                op = FixedTriangleMarkOperator()
                op.execute_with_location(bpy.context, (coords["x"], coords["y"], coords["z"]))
            else:
...
def start_websocket_server():
    global websocket_server, websocket_port
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    websocket_server = websockets.serve(handle, "localhost", websocket_port)
...
def stop_websocket_server():
    ...
    if websocket_server:
        # Close the server and reset the variable
...
if __name__ == "__main__":
    ...
    connect_to_webviewer=True
    if(connect_to_webviewer): 
        # Stop the server if it's already running
        stop_websocket_server()

        # Start the server in a new thread if the port is not in use
        if not is_port_in_use(websocket_port):
            thread = threading.Thread(target=start_websocket_server, daemon=True)
            thread.start()
            print(f"WebSocket server started on localhost:{websocket_port}")
        else:
            print(f"Port {websocket_port} is already in use.")
