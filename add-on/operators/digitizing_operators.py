...
class DrawStraightFatLineOperator(bpy.types.Operator):
    ...
    def draw_line(self, context, event):
        ...
        if self.prev_end_point:
            ...
            coord_3d_start = view3d_utils.region_2d_to_location_3d(
                region,
                region_3d,
            ...
            coord_3d_start.z += (
                extra_z_height  # Add to the z dimension to prevent clipping
            )

        coord_3d_end = view3d_utils.region_2d_to_location_3d(
            region,
            region_3d,
            (event.mouse_region_x, event.mouse_region_y),
            Vector((0, 0, 0)),
    ...
    def cancel(self, context):
        DrawStraightFatLineOperator._is_running = (
            False  # Reset the flag when the operator is cancelled
        )
...
class SimpleMarkOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if (
            ...
            if average_intensity > intensity_threshold:
                ...
                if (
                    average_intensity - intensity_threshold
                    > intensity_difference_threshold
                ) and show_popup:
                    bpy.ops.wm.correction_pop_up(
                        "INVOKE_DEFAULT",
                        average_intensity=average_intensity,
                        adjust_action="HIGHER",
                    )
                    return {"PASS_THROUGH"}
                # Region growing algorithm
                region_growth_coords, checked_indices = region_growing(
                    point_coords,
                    point_colors,
                    points_kdtree,
                    nearest_indices,
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == "VIEW_3D":
            SimpleMarkOperator._is_running = (
                True  # Set the flag to indicate the operator is running
            )
            context.window_manager.modal_handler_add(self)
            return {"RUNNING_MODAL"}
        else:
...
class ComplexMarkOperator(bpy.types.Operator):
    ...
    def process_location(
        ...
        print(
            "average color: ", average_color, "average intensity: ", average_intensity
        )
        ...
        if average_intensity > intensity_threshold:
            # if the average intensity is way higher than the threshold, give a warning
            if (
                average_intensity - intensity_threshold > intensity_difference_threshold
            ) and show_popup:
                bpy.ops.wm.correction_pop_up(
                    "INVOKE_DEFAULT",
                    average_intensity=average_intensity,
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == "VIEW_3D":
            ComplexMarkOperator._is_running = (
                True  # Set the flag to indicate the operator is running
            )
            context.window_manager.modal_handler_add(self)
            return {"RUNNING_MODAL"}
    ...
    def execute(self, context):
        ...
        if (
            hasattr(self, "external_x")
            and hasattr(self, "external_y")
            and hasattr(self, "external_z")
        ):
...
class FindALLRoadMarkingsOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        print(
            "Start auto detecting up to",
            markings_threshold,
            "road markings.. this could take a while",
        )
        ...
        for idx, color in enumerate(
            point_colors[::n][last_processed_index:], start=last_processed_index
        ):
            ...
            if intensity > intensity_threshold:
                ...
                if len(region_growth_coords) >= point_threshold:
                    ...
                    white_objects_count += (
                        1  # Increment counter when valid white object is found
                    )
...
class CurbDetectionOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS":
            ...
            if self.click_count == 1:
                ...
                print(
                    "second click at",
                    self.second_click_point,
                    "starting curb detection between mouseclicks",
                )
                start_time = time.time()
                curb_points, median_curb_point = self.detect_curb_points(
                    points_kdtree,
                    point_coords,
                ...
                if curb_points:
                    (
                        avg_lowest_point,
                        avg_highest_point,
                    ) = calculate_adjusted_extreme_points(curb_points)
                    ...
                    self.draw_curb_line(
                        self.first_click_point, self.second_click_point, extra_z_height
                    )
    ...
    def invoke(self, context, event):
        ...
        if CurbDetectionOperator._is_running:
            ...
            return self.cancel(
                context
            )  # Do not run the operator if it's already running
        self._is_running = True
        return {"RUNNING_MODAL"}

    # Detects the curb points between 2 mouseclicks, based on points without neighbors to the left and right
    def detect_curb_points(
        self, points_kdtree, point_coords, first_click_point, second_click_point
    ):
        ...
        half_num_samples = int(
            5 * line_length
        )  # Half the number of samples on each side
        ...
        def check_and_add_curb_points(sample_point):
            ...
            for idx in indices:
                ...
                left_neighbor = points_kdtree.query_ball_point(
                    point - perp_direction * neighbor_search_distance, 0.05
                )
                right_neighbor = points_kdtree.query_ball_point(
                    point + perp_direction * neighbor_search_distance, 0.05
                )
    ...
    def draw_curb_line(
        self, first_click_point, second_click_point, curb_height, median_curb_point=None
    ):
        ...
        adjusted_first_click = Vector(
            (first_click_point.x, first_click_point.y, curb_height)
        )
        adjusted_median_point = Vector(
            (median_curb_point.x, median_curb_point.y, curb_height)
        )
        adjusted_second_click = Vector(
            (second_click_point.x, second_click_point.y, curb_height)
        )
    ...
    def cancel(self, context):
        CurbDetectionOperator._is_running = (
            False  # Reset the flag when the operator is cancelled
        )
        print("Operator was properly cancelled")  # Debug message
        return {"CANCELLED"}
...
class SelectionDetectionOpterator(bpy.types.Operator):
    ...
    def find_white_objects_within_region(self, context, point_coords, point_colors):
        ...
        within_bbox = np.all(
            np.logical_and(min_corner <= point_coords, point_coords <= max_corner),
            axis=1,
        )
            ...
            while indices_to_check:
                ...
                if current_index not in checked_indices:
                    ...
                    if intensity > intensity_threshold:
                        ...
                        _, neighbor_indices = filtered_kdtree.query(
                            [filtered_points[current_index]], k=radius
                        )
                        indices_to_check.extend(
                            neighbor_index
                            for neighbor_index in neighbor_indices[0]
    ...
    def create_bounding_box(self, min_corner, max_corner):
        ...
        TriangleMarkOperator._is_running = (
            False  # Reset the flag when the operator is cancelled
        )
        print("Operator was properly cancelled")  # Debug message
        return {"CANCELLED"}
...
class AutoTriangleMarkOperator(bpy.types.Operator):
    ...
    bl_description = (
        "Marks multiple triangles by clicking on the first and last triangle"
    )
    _is_running = False  # Class variable to check if the operator is already running
    ...
    def modal(self, context, event):
        ...
        if (
            ...
            if region_growth_coords:
                filtered_triangle_coords = filter_noise_with_dbscan(
                    region_growth_coords,
                    eps=bpy.context.scene.filter_distance,
                    min_samples=bpy.context.scene.filter_neighbors,
                )
                ...
                create_shape(
                    filtered_triangle_coords,
                    shape_type="triangle",
                    vertices=triangle_vertices,
                )

                if (len(self._triangles)) >= 2:
                    outer_corners = self.find_closest_corners(
                        self._triangles[0], self._triangles[1]
                    )
                    self.perform_automatic_marking(
                        context,
                        intensity_threshold,
                        point_coords,
                        point_colors,
        ...
        for corner1 in triangle1:
            for corner2 in triangle2:
                distance = np.linalg.norm(
                    np.array(corner1[:2]) - np.array(corner2[:2])
    ...
    def perform_automatic_marking(
        self, context, intensity_threshold, point_coords, point_colors, points_kdtree
    ):
        line_points = []
        ...
        if line_points:
            create_polyline(
                line_points, "continuous_line"
            )  # create 1 line of out all the segments

    # Function to simulate a click and grow on a point
    def simulate_click_and_grow(
        self,
        location,
        context,
        intensity_threshold,
        ...
        if (
            average_intensity > intensity_threshold
        ) and not self._processed_indices.intersection(nearest_indices[0]):
            ...
            while indices_to_check:
                ...
                if current_index not in checked_indices:
                    ...
                    if intensity > intensity_threshold:
                        _, neighbor_indices = points_kdtree.query(
                            [point_coords[current_index]], k=50
                        )
                        indices_to_check.extend(
                            neighbor_index
                            for neighbor_index in neighbor_indices[0]
            ...
            if checked_indices:
                ...
                filtered_points = filter_noise_with_dbscan(
                    points,
                    eps=bpy.context.scene.filter_distance,
                    min_samples=bpy.context.scene.filter_neighbors,
                )
                ...
                create_shape(
                    filtered_points, shape_type="triangle", vertices=triangle_vertices
                )
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == "VIEW_3D":
            ...
            AutoTriangleMarkOperator._is_running = (
                True  # Set the flag to indicate the operator is running
            )
...
class TriangleMarkOperator(bpy.types.Operator):
    ...
    def create_triangle(
        ...
        location = region_2d_to_location_3d(
            context.region, context.space_data.region_3d, (x, y), (0, 0, 0)
        )
        region_growth_coords = []
        search_radius = 20
        ...
        if average_intensity > intensity_threshold:
            ...
            if region_growth_coords:
                ...
                filtered_current_triangle_coords = filter_noise_with_dbscan(
                    region_growth_coords,
                    eps=bpy.context.scene.filter_distance,
                    min_samples=bpy.context.scene.filter_neighbors,
                )
                self._processed_indices.update(checked_indices)
                current_triangle_vertices = create_flexible_triangle(
                    filtered_current_triangle_coords
                )
                ...
                if len(self._triangles) >= 2:
                    ...
                    if self._last_outer_corner is None:
                        outer_corners = self.find_outermost_corners(
                            self._triangles[-2], self._triangles[-1]
                        )
                        self._last_outer_corner = outer_corners[1]
                    else:
                        # Use the last outer corner and find the new one
                        new_outer_corner = self.find_outermost_corner(
                            self._triangles[-1], self._last_outer_corner
                        )
                    ...
                    if all(
                        isinstance(corner, (list, tuple)) for corner in outer_corners
                    ):
                        create_polyline(outer_corners, "triangles_base_line")
                        current_triangle_vertices = move_triangle_to_line(
                ...
                create_shape(
                    new_triangle_coords,
                    shape_type="triangle",
                    vertices=current_triangle_vertices,
                )
...
class AutoRectangleMarkOperator(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == "VIEW_3D":
            AutoRectangleMarkOperator._is_running = (
                True  # Set the flag to indicate the operator is running
            )
            context.window_manager.modal_handler_add(self)
            return {"RUNNING_MODAL"}
...
class SnappingLineMarkOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == "LEFTMOUSE" and is_mouse_in_3d_view(context, event):
            if event.value == "RELEASE":
                draw_line(
                    self, context, event, point_coords, point_colors, points_kdtree
                )
                return {"RUNNING_MODAL"}
        # If escape is pressed, stop the operator
        elif event.type == "RIGHTMOUSE" or event.type == "ESC":
...
class DashedLineMarkingOperator(bpy.types.Operator):
    ...
    bl_description = (
        "Finds and marks repeating dash lines after clicking on 2 dash lines"
    )
    ...
    def modal(self, context, event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS":
            ...
            if self.click_count == 1:
                self.first_cluster_center = self.find_cluster_center(
                    points_kdtree,
                    point_coords,
                    point_colors,
                    click_point,
                ...
                self.second_cluster_center = self.find_cluster_center(
                    points_kdtree,
                    point_coords,
                    point_colors,
                    click_point,
                ...
                if (
                    self.first_cluster_center is not None
                    and self.second_cluster_center is not None
                ):  # make sure the clusters are not empty
                    self.connect_clusters(
                        self.first_cluster_center, self.second_cluster_center
                    )
                    # Continue finding and connecting additional clusters
                    self.extend_dashed_lines(
                        points_kdtree,
                        point_coords,
                        point_colors,
                        self.second_cluster_center,
                        self.first_cluster_center,
                        intensity_threshold,
                    )
    ...
    def find_cluster_center(
        ...
        filtered_points = [
            point_coords[i]
            for i in indices
            if np.average(point_colors[i]) >= intensity_threshold
        ]
        print(
            "Number of cluster points found above ",
            int(intensity_threshold),
            f" intensity: {len(filtered_points)}",
        )
    ...
    def extend_dashed_lines(
        self,
        points_kdtree,
        point_coords,
        point_colors,
        start_cluster_center,
        end_cluster_center,
        intensity_threshold,
        search_radius=0.3,
    ):
        ...
        while True:
            ...
            new_cluster_center = self.find_cluster_center(
                points_kdtree,
                point_coords,
                point_colors,
    ...
    def cancel(self, context):
        DashedLineMarkingOperator._is_running = (
            False  # Reset the flag when the operator is cancelled
        )
        print("Operator was properly cancelled")  # Debug message
        return {"CANCELLED"}
...
from ..utils.digitizing_utils import (
    mark_point,
    create_shape,
    create_rectangle_line_object,
    create_polyline,
    create_flexible_triangle,
    create_dots_shape,
    draw_line,
    create_flexible_rectangle,
    create_fixed_square,
    draw_fixed_triangle,
)
